...
class IOHandler:
        ...
        for key in conf.cfg.keys():
            if len(conf.cfg.items(key)) > 0:
                ...
                if d[s.io_path_key] == "__name__":
                    d[s.io_path_key] = pkg_resources.resource_filename("Dkr5G", "")
                items.append(d)
        return cls(items)
    ...
    def define_object(self, obj: dict):
        ...
        create = not obj[s.io_exists_key] if s.io_exists_key in obj.keys() else False
    ...
    def evaluate_obj(self, obj: str) -> Tuple[str, str]:
        ...
        if obj in self.objects.keys():
            return obj, self.objects[obj].path
        return obj[1::], CI(obj)
