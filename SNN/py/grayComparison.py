...
parser.add_argument("--multiExperiment", dest="multiExperiment", default=False,
                    action="store_true", help="Activate the multi experiment flag")
...
parser.add_argument("-e", "--evaluation_file", dest="evaluation_file", default=None,
                    action="store", help="evaluation_comparison_file")
parser.add_argument("--evolution", dest="evolution_file", default=None,
                    action="store", help="evolution_comparison_file")
parser.add_argument("-p", "--phase", dest="phase", default=False,
                    action="store_true", help="Activate the flag for phase gray samples")
parser.add_argument("-t", "--title", dest="title", default="Video",
                    action="store", help="Activate the flag for phase gray samples")
...
def main():
    ...
    multiExperiment: bool = options.multiExperiment
    evaluation_file: str = options.evaluation_file
    ...
    custom_params = {'figure.figsize':(8,4), "font.size": 44}
    ...
    sl_df = sl_df[sl_df["VMAF"] == 80].copy()
    sl_df.drop(["VMAF", "Origin", "Phase"], axis=1, inplace=True)
    # print(sl_df)
    sl_df["Value"] = sl_df.groupby(["Experiment", "Evaluation"])["Value"].transform(lambda x: x/sum(x)).reset_index()["Value"].values
    # print(sl_df)
    # print(sl_df[sl_df["Experiment"] == 0])

    p = plt(sl_df, format=["pdf", "png"])
    p.sns_set(font_scale=2.0)
    p.sns_set_api(sns.set_style, "white")
    p("line", x="Evaluation", y="Value", hue="Stat", linewidth=2.0)
    p.plot.set_xticks(range(0,9), labels=range(1,10))
    p.plot.set_yticks([x/100.0 for x in range(0, 101, 20)], labels=range(0,101,20))
    p.set(xlabel="SL Iteration",
          ylabel=r"$\hat{U}$ Samples [%]")
    p.set_legend(ncol=3, y=0.5, title="Labels assignment", title_fontsize=18, loc="center", prop={'size': 16})
    print(output.path)
    p.save(f"{output.path}/grays_stats_evolution-VMAF80.pdf")
    ...
    # for vmaf in [80, 90, 99]:
    #     tmp_acc_df = accuracy_df[accuracy_df["VMAF"] == vmaf].copy()
    #     # tmp_acc_df = tmp_acc_df[tmp_acc_df["Experiment"] != 17]
    #     zero_exp = tmp_acc_df[tmp_acc_df["Value"] == 0]["Experiment"].unique()
    #     print(zero_exp)
    #     tmp_acc_df = tmp_acc_df[~tmp_acc_df["Experiment"].isin(zero_exp)]
    #     last_round_acc = tmp_acc_df[tmp_acc_df["Evaluation"] == 9]
    #     first_round_acc = tmp_acc_df[tmp_acc_df["Evaluation"] == 0]
    #     remove_exp = []
    #     for e in tmp_acc_df["Experiment"].unique():
    #         if len(first_round_acc[first_round_acc['Experiment'] == e]['Value'].values) == 0 or \
    #             len(last_round_acc[last_round_acc['Experiment'] == e]['Value'].values) == 0:
    #             remove_exp.append(e)
    #     tmp_acc_df = tmp_acc_df[~tmp_acc_df["Experiment"].isin(remove_exp)]

    #     last_round_acc = tmp_acc_df[tmp_acc_df["Evaluation"] == 9]
    #     first_round_acc = tmp_acc_df[tmp_acc_df["Evaluation"] == 0]
    #     for e in tmp_acc_df["Experiment"].unique():
    #         first = first_round_acc[first_round_acc['Experiment'] == e]['Value'].values[0]
    #         last= last_round_acc[last_round_acc['Experiment'] == e]['Value'].values[0]
    #         print(f"exp-{e}: {first} -> {last}, trend: {last > first}")

    #     p = plt(tmp_acc_df, format=["pdf", "png"])
    #     p.sns_set(font_scale=2.25)
    #     p.sns_set_api(sns.set_style, "white")
    #     p("line", x="Evaluation", y="Value", linewidth=2.0)
    ...
    p("line", x="Evaluation", y="Value", hue="VMAF", markers=True, dashes=True)
    p.set(xlabel="Evaluation cycle",
          ylabel=f"Accuracy [%]",
...
"""
        ...
        def res_plot(df, plt_name, output_name, y_label):
            p = plt(df, format=["pdf", "png"], palette=cm)
            p.sns_set(font_scale=2.25)
            p.sns_set_api(sns.set_style, "white")
            p(plt_name, x="VMAF", y="Value", hue="Origin",
              hue_order=["Baseline", "SL", "ATELIER"])
            p.set(xlabel=r"$v_{min}$",
                  ylabel=y_label,
                  ylim=(0.0, 100.01))
            p.set_legend(ncol=3, y=0.05, fontsize='18',
                         title_fontsize='25')
            p.save(f"{output.path}/{output_name}.pdf")

        res_plot(default_sl, "bar", "accuracy_bar_plot", f"Accuracy [%]")
        res_plot(precision_df, "bar", "precision_bar_plot", r"Precision $\frac{TP}{TP+FP}$ [%]")
        res_plot(default_sl, "boxplot", "accuracy_box_plot", f"Accuracy [%]")
        res_plot(precision_df, "boxplot", "precision_box_plot", r"Precision $\frac{TP}{TP+FP}$ [%]")
        res_plot(default_sl, "violin", "accuracy_violin_plot", f"Accuracy [%]")
        res_plot(precision_df, "violin", "precision_violin_plot", r"Precision $\frac{TP}{TP+FP}$ [%]")
