...
class RL_AC_manager(RLModelHandler):
    ...
    def calculate_reward(self, labels: tf.Tensor,
                         current_params: Optional[tf.Tensor] = None,
                         previous_params: Optional[tf.Tensor] = None) -> tf.Tensor:
        self.write_msg(f"Reward function")
        cf_matrix = self.calculate_confusion_matrix(labels)
        # train_matrix = self.calculate_confusion_matrix(train_labels, exp_l=train_exp_labels)
        # test_matrix = self.calculate_confusion_matrix(test_labels, exp_l=test_exp_labels)

        previous_accuracy = None
        self.write_msg(f"Previous reward: {previous_accuracy}, type: {type(previous_accuracy)}")
        if self.history["previous_accuracy"].history is not None:
            previous_accuracy = round(float(self.history["previous_accuracy"].history[-1].numpy()), 4)
            self.write_msg(f"Previous reward: {previous_accuracy}, type: {type(previous_accuracy)}")

        reward, accuracy = self.reward_function(cf_matrix,
                                                current_params.numpy(),
                                                previous_params=previous_params.numpy(),
                                                previous_accuracy=previous_accuracy)
        self.write_msg(f"Reward obtained: {reward}, Accuracy: {accuracy}")
        # reward, accuracy = self.reward_function(cf_matrix,
        #                                         previous_accuracy=previous_accuracy)

        self.register("RewardAccuracy", accuracy, step=self.current_step-1)
        self.register("PreviousAccuracy", previous_accuracy, step=self.current_step-1)
        self.write_msg(f"Reward after comparison: {reward}")

        tf_reward = tf.convert_to_tensor([reward], dtype=tf.float32)
        tf_accuracy = tf.convert_to_tensor([accuracy], dtype=tf.float32)
        ...
        self.write_msg(f"Accuracy from the cm: {(cf_matrix['TP'] + cf_matrix['TN'])/sum(cf_matrix.values())}")
    ...
    def step(self, observation: tf.Tensor,
             labels: tf.Tensor,
             game_over: bool) -> Optional[int]:
        self.write_msg(f"Step function, step: {self.current_step}")
        observation = self.observation_preproc(observation,
        ...
        if self.previous_state is not None:
            ...
            current_params = observation[0]
            previous_params = self.previous_state[0]
            reward = self.calculate_reward(labels,
        ...
        # np_val, tf_val = self.action_policy(observation,
        #                                     model=self.model,
