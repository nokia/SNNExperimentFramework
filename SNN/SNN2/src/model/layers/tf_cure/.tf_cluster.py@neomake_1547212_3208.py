...
def tf_concat(points, slice_idx) -> tf.Tensor:
    return tf.concat([points[:slice_idx], points[slice_idx+1:]], 0)
...
@tf.function(input_signature=(tf.RaggedTensorSpec(shape=[None, None, None],
                              ...
                              tf.TensorSpec(shape=[], dtype=tf.int64)))
def ragged_tf_concat(points, slice_idx) -> tf.Tensor:
    return tf.concat([points[:slice_idx], points[slice_idx+1:]], 0)
...
class tf_clusters():
    ...
    def best_closest(self,
        ...
        with timeit("Compute closest distance manual", active=self.timeit_flag):
    ...
    def compute_representors(self, c_id:int) -> None:
        ...
        if c_points.shape[0] > self.n_rep:
            with timeit("Representors selection algorithm", active=False):
    ...
    def __delitem__(self, id: int) -> None:
        ...
        None
        """
        idx = tf.constant(self.get_idx(id))
        with timeit("Ragged concat"):
            self.__r_clusters = ragged_tf_concat(self.clusters, idx)
        with timeit("Ragged concat"):
            self.__r_reps = ragged_tf_concat(self.reps, idx)
        with timeit("tf concat"):
            self.__ids = tf.cast(tf_concat(tf.cast(self.ids, tf.float32), idx), tf.int32)
        with timeit("tf concat"):
            self.__closest = tf.cast(tf_concat(tf.cast(self.closest, tf.float32), idx), tf.int32)
        with timeit("tf concat"):
        self.__distances = tf_concat(self.distances, idx)
