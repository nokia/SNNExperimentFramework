...
def glob_dst(local: tf.Tensor,
    ...
    dst = tf.math.square(tf.norm(tf.math.subtract(local_rep, remote_rep), axis=-1))
...
class tf_clusters():
    ...
    def compute_representors(self, c_id:int) -> None:
        ...
        if c_points.shape[0] > self.n_rep:
            with timeit("Representors selection algorithm", active=True):
    ...
    def select_representors(self, points: tf.Tensor) -> tf.Tensor:
        ...
        while i <= self.n_rep:
            with timeit("Select adv", active=time_flag):
                adv = tmp_rep if not tmp_rep is None else tf.expand_dims(tf.reduce_mean(points, axis=0), 0)
            # with timeit("Compute adv-rep difference", active=time_flag):
            #     reps = tf.ones([tf.shape(adv)[0]], dtype=tf.int32)
            #     print(reps)
            #     reps[0] += self.n_rep - tf.shape(adv)[0]
            #     print(reps)
            #     adv = tf.repeat(adv, repeats=reps, axis=0)
            #     print(adv)
            # print(self.n_rep)
            # print(tf.shape(adv)[0])
            # adv_shape = tf.shape(adv)
            # diff = self.n_rep - adv_shape[0] + 1
            # print(diff)
            # rep = tf.range(adv_shape[0])
            # print(rep)
            ...
            with timeit("Compute distances", active=time_flag):
                all_dst = pp_dst(points, adv)

            with timeit("Select best distance", active=time_flag):
            ...
            with timeit("Concatenate representors", active=time_flag):
