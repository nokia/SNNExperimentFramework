...
class tf_clusters():
    ...
    def merge(self, u_id: int, v_id: int) -> None:
        ...
        None
        """
        with timeit("Safty checks"):
            if u_id == v_id:
                raise ValueError("Cannot merge the same cluster")
        # Merge the clusters points from v_idx into u_idx

        with timeit("init idx"):
            idx = static_get_idx(u_id, self.ids)
            # v_idx = static_get_idx(v_id, self.ids)
            print(idx)
        raise Exception

        # with timeit("Merge clusters points"):
        ...
        # with timeit("Merge swap u cluster with new one"):
