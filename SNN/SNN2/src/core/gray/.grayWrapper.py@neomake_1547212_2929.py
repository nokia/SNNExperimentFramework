...
# from SNN2.src.model.layers.tf_cure.tf_cluster import pp_dst
...
def gray_update_margin_general_cure(model,
    ...
    while True:
        ...
        # Compute centroids
        g_cure = TfCURE(good_emb, 1)
        g_cure.process()
        print(g_cure.clusters.reps.numpy())

        b_cure = TfCURE(bad_emb, 1)
        b_cure.process()
        print(b_cure.clusters.reps.numpy())
        ...
        # diff_good_dst = pp_dst(diff_emb, tf.concat([g_cure.clusters.reps, g_cure.clusters.mean], 0))
        # diff_bad_dst = pp_dst(diff_emb, tf.concat([b_cure.clusters.reps, b_cure.clusters.mean], 0))
