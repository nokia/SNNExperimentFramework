...
def main():
    ...
    if options.inference is not None:
        ...
        print(pp.data_prop["Targets"]["tf_values"])
        print(tf.where(pp.data_prop["Targets"]["tf_values"] < 20))
        print(pp.data_prop["ExpID"]["tf_values"])
        objects = [pp.data_prop["Targets"]["tf_values"],
                   pp.data_prop["ExpID"]["tf_values"].numpy()]
        labels = ["BADDIFF-target-numpy",
                  "BADDIFF-ExpID-numpy"]
