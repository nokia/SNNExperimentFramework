...
output_file="${output}/merged_reinforcement_step_${appendix}.csv"
for folder in ${root}/*
do
		if [[ -d ${folder} ]]; then
				file="${folder}/csv/reinforcement_step_${appendix}.csv"
				cp ${file} "${file}_awk.csv"
		fi
done

merge ${output_file} ${root} "csv" "reinforcement_step_" "_awk.csv"

if $clean; then
		for folder in ${root}/*
		do
				if [[ -d ${folder} ]]; then
						file="${folder}/csv/reinforcement_step_${appendix}.csv_awk.csv"
						mv ${file} /tmp/
				fi
		done
fi
